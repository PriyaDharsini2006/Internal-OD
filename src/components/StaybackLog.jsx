...
const StaybackLog = ({ staybacks, setStaybacks, fetchStaybacks }) => {
  ...
  const openStaybackDetails = async (stayback) => {
    try {
      setSelectedStayback(stayback);
      setStudents(stayback.students || []);
      setCurrentPage(1);
      setSearchTerm('');
    } catch (error) {
      console.error('Failed to fetch stayback students', error);
    }
  };
  ...
  const addStudentToStayback = async (email) => {
    ...
    try {
      const response = await fetch(`/api/staybacks/${selectedStayback.id}/students`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, staybackId: selectedStayback.id })
      });
      ...
      if (response.ok) {
        ...
        setStaybacks(prevStaybacks => 
          prevStaybacks.map(stayback => 
            stayback.id === selectedStayback.id 
              ? { ...stayback, students: updatedStudents } 
              : stayback
          )
        );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {staybacks.map(stayback => (
          <div 
            key={stayback.id} 
            className={`p-4 rounded-md shadow-md cursor-pointer ${
              new Date() > new Date(stayback.date) 
                ? 'bg-green-100 hover:bg-green-200' 
                : 'bg-yellow-100 hover:bg-yellow-200'
            }`}
            onClick={() => openStaybackDetails(stayback)}
          >
            <h3 className="font-semibold text-lg">{stayback.title}</h3>
            <p>Team: {stayback.team}</p>
            <p>Date: {new Date(stayback.date).toLocaleDateString()}</p>
          </div>
        ))}
      </div>
      ...
      {selectedStayback && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-11/12 max-w-7xl max-h-[90vh] flex flex-col">
            <h2 className="text-2xl font-bold mb-6">{selectedStayback.title} Details</h2>
