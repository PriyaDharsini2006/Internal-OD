...
import { User, CalendarDays, Printer, Menu, X, Trash2 } from 'lucide-react';

export const MeetingLog = ({ meetings, setMeetings, fetchMeetings }) => {
  ...
  return (
    <div className="print-container container mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            <div className="text-sm text-white">
                              From: {formatTime(meeting.from_time)}
                            </div>
                            <div className="text-sm text-white">
                              To: {formatTime(meeting.to_time)}
                            </div>
                          </div>
                        </div>
                      </td>
      ...
      {/* Meeting Details Modal/Print View */}
      {selectedMeeting && (
        <div className="print-view hidden print:block">
          ...
          <div className="first-page print-page">
            ...
            <div className="meeting-details leading-relaxed text-black mb-6 sm:mb-10 text-sm sm:text-base">
              <h2 className="text-xl font-bold mb-4">{selectedMeeting.title}</h2>
              <p><strong>Team:</strong> {selectedMeeting.team}</p>
              <p><strong>Date:</strong> {new Date(selectedMeeting.date).toLocaleDateString()}</p>
              <p><strong>Time:</strong> {formatTime(selectedMeeting.from_time)} - {formatTime(selectedMeeting.to_time)}</p>
              <p><strong>Location:</strong> {selectedMeeting.location || 'Not specified'}</p>
            </div>

            <div className="signature-section mt-8 flex flex-col items-end justify-end">
              <div className="text-sm md:text-base text-center">
                <p className="text-black m-0">Meeting Organizer</p>
                <p className="text-black m-0">{selectedMeeting.organizer || 'Not specified'}</p>
              </div>
            </div>
          </div>

          {/* Second Page - Attendees Table */}
