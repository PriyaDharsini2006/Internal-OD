...
const MeetingCountLeaderboard = () => {
    ...
    useEffect(() => {
        const fetchMeetingUsers = async () => {
            try {
                const response = await fetch('/api/meetingcnt');
                if (!response.ok) {
                    throw new Error('Failed to fetch meeting users');
                }
        ...
        <div className="print-container container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-row  text-center">
                <img
                    ...
                    alt="Company Logo"
                />
                <p className='font-grotesk text-3xl mt-[50px] ml-[350px] justify-center text-[#00f5d0]'>Meeting Counts</p>
            </div>
            <div className="browser-view backdrop-blur-xl rounded-2xl border border-white/10">
