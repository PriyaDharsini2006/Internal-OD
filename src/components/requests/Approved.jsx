...
export const Approved = () => {
  ...
  const sendExcelToEmailAPI = async () => {
    setIsLoading(true);
    setStatus('');

    try {
      // Fetch Excel from first deploy link
      const excelResponse = await fetch('https://internal-od.vercel.app/teamlead/api/generate-excel', {
        method: 'GET'
      });
      ...
      // Prepare FormData to send to email API
      const formData = new FormData();
      formData.append('excelFile', excelBlob, 'approved_requests.xlsx');
      ...
      setStatus('An error occurred while processing emails');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="print-container container mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <h1 className="text-2xl font-bold mb-4">Send Approved Requests</h1>
      <button 
        onClick={sendExcelToEmailAPI}
        disabled={isLoading}
        className="w-full bg-blue-500 text-white p-2 rounded 
          hover:bg-blue-600 disabled:bg-blue-300"
      >
        {isLoading ? 'Processing...' : 'Send Approved Requests'}
      </button>
