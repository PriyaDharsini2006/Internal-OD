...
export async function POST(req, { params }) {
  try {
    ...
    const { students } = await req.json(); // Change to receive an array of students
    ...
    const updatedStudents = [...new Set([...meeting.students, ...students])];
    ...
    console.error('Detailed error:', error);
    return NextResponse.json(
      { error: 'Failed to add students to meeting', details: error.message }, 
      { status: 500 }
    );
  } finally {
    await prisma.$disconnect();
